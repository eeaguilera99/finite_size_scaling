...
plot!(plt1, t_vals, model(log.(t_vals),fit_params1), xscale=:log10, label=latexstring("\$d≈$(round(2/fit_params1[1], digits=3))\$"))
...
plot!(plt2, t_vals, model(log.(t_vals),fit_params2), xscale=:log10, label=latexstring("\$d≈$(round(2/fit_params2[1], digits=3))\$"))
