...
# Perform collapse
res, shifts, X, Y, Yerr, s_rel = finite_time_scaling(K_vals, t_vals, p2_mat, p2_err_mat; d=dim, n_kicks_i=1, n_kicks_f=0)

# Plot before collapse
plt1 = plot(title="Raw data",
    xlabel="ln(t^(-1/d))", ylabel="ln(Λ)")
for (i,K) in enumerate(K_vals)
    plot!(plt1, X[:], Y[i,:], marker=:o, label="K="*string(round(K, digits=3)))#, yerror=Yerr[i,:]
end
display(plt1)
...
for (i,K) in enumerate(K_vals)
    plot!(plt2, X[:] .+ shifts[i], Y[i,:], marker=:o, label="")
end
display(plt2)
