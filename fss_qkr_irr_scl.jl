...
function χ(K)
    return (b0*-(Kc-K) + b1*(-(Kc-K))^2)# + b1*((Kc-K)/Kc)^2
end
...
scatter!(vec(X1 .+ shifts) .+ 2.2, vec(Y1), mc=:blue, label="Uncorrected")
#xlims!(-1, 7)
display(plt2)
...
plt5 = plot(title="One-parameter scaling collapse vs Corrections",
    xlabel=latexstring("ln \$(ξ/N^{1/d})\$"), ylabel=latexstring("ln \$(Λ)\$"))
scatter!(plt5, vec(X1 .+ [fit_shifts[i] for i in 1:length(K_vals), t in 1:size(X1,2)]), vec(Y1), mc=:red, marker=:o, 
